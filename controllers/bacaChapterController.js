...
const getBacaChapter = async (req, res) => {
  try {
    ...
    const images = [];
    // Menangkap elemen gambar di area baca dan menyaring aset sampah web
    $("#Baca_Komik img, img.ww, img[id], .pembaca img, article img, div[id*='chapter'] img").each((_, el) => {
      ...
      // Memastikan hanya gambar chapter asli yang masuk (menyingkirkan ikon bendera/asset dan base64)
      if (
        src &&
        ...
        (src.includes("part") || /\/\d+(_|\.|\/)/.test(src))
      ) {
        images.push({
          src,
