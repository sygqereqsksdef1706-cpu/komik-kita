...
const getBacaChapter = async (req, res) => {
  try {
    ...
    const images = [];
    // Menangkap gambar di area baca dengan filter anti-bendera/asset web
    $("#Baca_Komik img, img.ww, img[id], .pembaca img, article img, div[id*='chapter'] img").each((_, el) => {
      const img = $(el);
      ...
      // Memastikan gambar adalah bagian chapter komik asli dan membersihkan aset bendera UI
      if (
        src &&
        /komiku\.org/i.test(src) &&
        ...
        !src.includes("kr.png") &&
        !src.includes("jp.png") &&
        (src.includes(slug) || src.includes("part") || /\/\d{4}-\d{2}-\d{2}\//.test(src))
      ) {
        images.push({
          src,
          alt: normalizeText(img.attr("alt")) || "Chapter Image",
          id: normalizeText(img.attr("id")) || "",
