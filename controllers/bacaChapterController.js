...
const getBacaChapter = async (req, res) => {
  try {
    ...
    const images = [];
    // Menangkap semua elemen gambar di area baca dengan dukungan lazy-load menyeluruh
    $("#Baca_Komik img, img.ww, img[id], .pembaca img, article img, div[id*='chapter'] img").each((_, el) => {
      ...
      // Memastikan semua gambar dari komiku.org dengan ekstensi file valid tertangkap (mendukung format tanggal/part)
      if (
        src &&
